			...
			if(!empty($firstname) && !empty($lastname) && !empty($password) && !empty($email) && !empty($gender) && !empty($dob) && !empty($month) && !empty($year)  && !empty($password1)/*&& !empty($response)*/)
			{
				...
				if($password==$password1)
				{
					...
					else
					{
						...
						$query="INSERT INTO login VALUES('','".mysqli_real_escape_string($con,$password)."','".mysqli_real_escape_string($con,$firstname)."','".mysqli_real_escape_string($con,$lastname)."','".mysqli_real_escape_string($con,$email)."','".mysqli_real_escape_string($con,$gender)."','".mysqli_real_escape_string($con,$dob)."','".mysqli_real_escape_string($con,$date1)."','','','','','','','','','','','','','','','.$ip.','1','','','','".mysqli_real_escape_string($con,$amount)."')";
